...
部署脚本强制 API 监听 `127.0.0.1:8088`；不要将 8088 端口暴露到公网。

```bash
cd /opt/Spider_XHS/deploy
chmod +x startup.sh stop.sh restart.sh

# 创建或复用 .venv、安装依赖，启动仅监听本机的 API 并发布 Nginx 配置
./startup.sh
...
```bash
curl https://spider-xhs-api.winzyy.com/internal/v1/metrics/overview
```
